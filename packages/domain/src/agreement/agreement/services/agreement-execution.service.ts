...
export interface ExecutionDecision {
  ...
  inExecutionStatus:
    | AgreementStatusEnum.IN_EXECUTION
    | AgreementStatusEnum.IN_EXECUTION_WITH_DELAY;
}
...
export class AgreementExecutionService {
   ...
   */
  static computeExecutionDecision(
   ...
   */
  static validateExecutionPeriod(
    contractPeriod: ContractPeriod,
    executionPeriod: ExecutionPeriod,
  ): void {
    if (executionPeriod.startDate < contractPeriod.signatureDate) {
      throw new Error(
        'Execution cannot start before the contract signature date',
      );
    }
  }
}
