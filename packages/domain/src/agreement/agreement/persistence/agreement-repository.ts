...
export interface AgreementRepository {
  ...
  findPaginatedWithFilters(
    params: AgreementFilterParams,
    user: AgreementUserContext,
  ): Promise<{ data: AgreementRecord[]; total: number }>;
  getAgreementsStatusStats(
    params: { startDate?: Date; endDate?: Date },
    user: AgreementUserContext,
  ): Promise<{ status: string; total: string }[]>;
  getAgreementsTypeStats(user: AgreementUserContext): Promise<{ type: string; total: string }[]>;

  // ── Write ──────────────────────────────────────────────────────────────
  /** Creates a new agreement record with direction/departement/vendor IDs. */
  createRecord(
    data: Omit<AgreementRecord, 'id' | 'direction' | 'departement' | 'vendor'> & {
      directionId: string;
      departementId: string;
      vendorId: string;
    },
