...
} from '@/features/direction/queries/direction.queries';
...
const DirectionContent = () => {
  ...
  const totalDepts = directions.reduce(
    (sum, d) => sum + (d.departements?.length ?? 0),
    0,
  );
  const totalUsers = directions.reduce(
    (sum, d) =>
      sum +
      (d.departements?.reduce(
        (s: number, dept: any) => s + (dept.users ?? 0),
        0,
      ) ?? 0),
    0,
  );
  ...
  const handleDeleteDepartement = (id: string) => {
    deleteDepartement(id, {
      onSuccess: () =>
        showSnackbar({
          message: 'la suppression de departement a reusi',
          severty: 'success',
        }),
      onError: (err: any) =>
        showSnackbar({
  ...
  const handleDeleteDirection = (directionId: string) => {
    deleteDirection(directionId, {
      onSuccess: () => {
        showSnackbar({
          message: 'La direction a été supprimée avec succès',
          severty: 'success',
        });
  ...
  const handleShowDepartementUsers = (
          ...
          <div className={styles.directionsWrapper}>
            {[...Array(4)].map((_, i) => (
              <Skeleton
                ...
                animation="wave"
                height={56}
                sx={{ borderRadius: 0, mb: '1px', display: 'block' }}
              />
            ))}
          </div>
        </div>
  ...
  return (
    <div id="directions-page" className={styles.container}>
          ...
          {directions.map((direction, index) => (
            <Accordion
              ...
              sx={{
                ...
                '&.Mui-expanded': {
                  borderColor: 'var(--navy-mid, #17498E) !important',
                },
              }}
            >
              <AccordionSummary
                expandIcon={
                  <ExpandMoreIcon
                    sx={{ color: 'var(--text-muted, #96A3B5)', fontSize: 20 }}
                  />
                }
              >
                  ...
                  <div className={styles.accordionLeft}>
                    ...
                    {direction?.departements?.length > 0 && (
                      <span className={styles.deptCountBadge}>
                        {direction.departements.length} dept
                        {direction.departements.length > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                    ...
                    <table className={styles.deptTable}>
                      <thead>
                        <tr>
                          ...
                          {dispalayIfAdmin() && (
                            <th className={styles.center}>Supprimer</th>
                          )}
                        </tr>
                      </thead>
                      <tbody>
                            ...
                            {dispalayIfAdmin() && (
                              <td className={styles.center}>
                                <Button
                                  ...
                                  className={styles.deleteBtn}
                                  onClick={() =>
                                    handleDeleteDepartement(row?.id ?? '')
                                  }
                                  size="small"
                                >
                                  <DeleteIcon sx={{ fontSize: 16 }} />
      ...
      <Modal
        ...
        onClose={() => setOpenDirectionModal(false)}
      >
        <CreateDirection
          pushDirection={() => {}}
          handleDirectionModalClose={() => setOpenDirectionModal(false)}
        />
      </Modal>

      <Modal
        open={openDepartementUsers}
        onClose={() => setOpenDepartementUsers(false)}
      >
        <DepartementUsersList
          departementId={choosenDepartementId}
          departementName={choosenDepartementName}
          handleClose={() => setOpenDepartementUsers(false)}
        />
      </Modal>
