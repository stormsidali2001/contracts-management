...
function CompactCard({ stats, cardId, setExpanded }: any) {
  ...
  if (!stats) {
    return (
      <motion.div
        layoutId={`expandableCard-${cardId}`}
    ...
    datasets: [
      {
        ...
        borderColor: tokens.color.navyMid,
        borderWidth: 1.5,
        hoverBackgroundColor: tokens.color.navyLight,
        hoverBorderColor: tokens.color.navyMid,
    ...
    >
      <div className={styles.cardHeader}>
        <div className={styles.cardHeaderLeft}>
          ...
          {latestCount !== null && (
            <span className={styles.latestCount}>{latestCount} actifs</span>
          )}
        </div>
