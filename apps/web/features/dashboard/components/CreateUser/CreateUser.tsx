...
const STEP_META = [
  {
    title: 'Identifiants',
    subtitle: 'Nom, prénom et adresse email',
    Icon: PersonOutlinedIcon,
  },
  {
    title: 'Profil',
    subtitle: 'Photo de profil et rôle',
    Icon: BadgeOutlinedIcon,
  },
  {
    title: 'Direction',
    subtitle: 'Direction et département',
    Icon: AccountTreeOutlinedIcon,
  },
  {
    title: 'Validation',
    subtitle: 'Création du compte',
...
const CreateUser = ({ handleClose }: Proptype) => {
  ...
  const handleDirectionChange = (event: SelectChangeEvent) => {
    ...
    const departement =
      direction.departements[
        Math.floor(Math.random() * (direction.departements.length - 1))
      ];
    setSelectedDepartement({
      label: departement?.title,
      value: departement.id as string,
  ...
  const handleSubmit = async () => {
    ...
    const isExeptionRoles = (value: string) =>
      ['ADMIN', 'JURIDICAL'].some((el) => el === value);
    ...
    createUser(newUser, {
      onSuccess: () =>
        showSnackbar({
          message: "l'utilisateur a eté creé avec success.",
          severty: 'success',
        }),
      onError: (err: any) =>
        showSnackbar({
          ...
          {steps.map((_, index) => (
            <>
              <div
                ...
                onClick={() =>
                  !nextBtnshouldBeDisabled() && handleStepLabel(index)
                }
              >
              ...
              {index < steps.length - 1 && (
                <div
                  className={`${styles.stepLine} ${index < activeStep ? styles.stepLineDone : ''}`}
                  key={`line-${index}`}
                />
              )}
            </>
      ...
      <div className={styles.body}>
        ...
        {activeStep === 0 && (
          <div className={styles.fieldGroup}>
            <p className={styles.stepHint}>
              Renseignez les informations d'identification du nouveau compte.
            </p>
            <TextField
              value={firstName}
              ...
              helperText={firstNameShouldDisplayError && 'Minimum 5 caractères'}
              size="small"
              label="Nom"
              fullWidth
            />
            <TextField
              ...
              helperText={lastNameShouldDisplayError && 'Minimum 5 caractères'}
              size="small"
              label="Prénom"
              fullWidth
            />
            <TextField
              ...
              helperText={emailShouldDisplayError && 'Adresse email non valide'}
              size="small"
              label="Email"
              type="email"
              fullWidth
            ...
            <div className={styles.profileTopRow}>
              ...
              <div className={styles.profilePreview}>
                <div className={styles.profilePreviewAvatar}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={profileImgPreview || '/blank-profile-picture.png'}
                    alt="Profil"
                  />
                </div>
                <div className={styles.profilePreviewInfo}>
                  <span className={styles.profilePreviewName}>
                    {firstName} {lastName}
                  </span>
                  <span className={styles.profilePreviewRole}>
        ...
        {activeStep === 2 && (
          <div className={styles.directionStep}>
            <p className={styles.stepHint}>
              Associez l'utilisateur à une direction et un département.
            </p>
            <FormControl fullWidth size="small">
              ...
              >
                {directions.map((dr, index) => (
                  <MenuItem value={dr?.id ?? ''} key={index}>
                    {dr.abriviation ?? ''}
                  </MenuItem>
                ))}
              </Select>
            ...
            {isImageUploading ? (
              <>
                <div
                  ...
                  style={{ background: 'rgba(23,73,142,0.08)' }}
                >
                  <CloudUploadOutlinedIcon
                    sx={{ fontSize: 28, color: 'var(--navy-mid)' }}
                  />
                </div>
