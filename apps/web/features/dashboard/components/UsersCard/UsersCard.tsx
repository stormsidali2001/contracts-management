...
function CompactCard({ stats, setExpanded, cardId }: any) {
  const getEntityPercentage = (entity: string) => {
    ...
    switch (entity) {
      case 'admin':
        return Math.round((100 * stats.admin) / stats.total);
      case 'juridical':
        return Math.round((100 * stats.juridical) / stats.total);
      case 'employee':
        return Math.round((100 * stats.employee) / stats.total);
    }
    return 0;
  };
  const percentages = [
    {
      percentage: getEntityPercentage('juridical'),
      icon: JuridicalIcon,
      name: 'juridique',
    },
    {
      percentage: getEntityPercentage('employee'),
      icon: EmployeeIcon,
      name: 'employé',
    },
    {
      percentage: getEntityPercentage('admin'),
      icon: AdminIcon,
      name: 'admin',
    },
  ];
