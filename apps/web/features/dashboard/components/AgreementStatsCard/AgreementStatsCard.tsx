  ...
  if (expanded) {
    return (
      ...
      >
        <Doughnut
          data={chartData}
          options={{ maintainAspectRatio: false, layout: { padding: 10 } }}
        />
      </ExpandedOverlay>
    );
  }
      ...
      <div className={styles.statusBody}>
        <div className={styles.statusTiles}>
          ...
          <div className={styles.statusTile}>
            ...
            <span
              className={`${styles.statusTileCount} ${(stats?.status?.not_executed ?? 0) > 0 ? styles.countError : ''}`}
            >
          ...
          <div className={styles.statusTile}>
            ...
            <span className={styles.statusTileCount}>
              {(stats?.status?.executed ?? 0) +
                (stats?.status?.executed_with_delay ?? 0)}
            </span>
          ...
          <div className={styles.statusTile}>
            ...
            <span className={styles.statusTileCount}>
              {(stats?.status?.in_execution ?? 0) +
                (stats?.status?.in_execution_with_delay ?? 0)}
            </span>
...
function TypeCard({ stats }: any) {
  ...
  const total = (stats?.types?.contract ?? 0) + (stats?.types?.convension ?? 0);
  const contractPct =
    total === 0 ? 0 : Math.round((stats.types.contract / total) * 100);
  const convensionPct =
    total === 0 ? 0 : Math.round((stats.types.convension / total) * 100);
      ...
      <div className={styles.typeBody}>
        <div className={`${styles.typeCard} ${styles.contractTile}`}>
          <div className={styles.typeTopRow}>
            <div
              className={`${styles.typeIconWrap} ${styles.contractIconWrap}`}
            >
              <ContractFilledIcon />
            </div>
          ...
          <span className={styles.typeCount}>
            {stats?.types?.contract ?? 0} accords
          </span>
        ...
        <div className={`${styles.typeCard} ${styles.convensionTile}`}>
          <div className={styles.typeTopRow}>
            <div
              className={`${styles.typeIconWrap} ${styles.convensionIconWrap}`}
            >
              <ConvensionFilledIcon />
            </div>
            <span className={styles.typeLabel}>Convention</span>
          </div>
          <span className={`${styles.typePct} ${styles.typePctGreen}`}>
            {convensionPct}%
          </span>
          <span className={styles.typeCount}>
            {stats?.types?.convension ?? 0} accords
          </span>
          <div className={styles.typeBar}>
            <div
              className={`${styles.typeBarFill} ${styles.typeBarFillAlt}`}
              style={{ width: `${convensionPct}%` }}
            />
          </div>
        </div>
      </div>
...
function DirectionsCard({ stats }: any) {
  ...
  const maxCount = Math.max(
    ...(stats?.topDirections?.map((d: any) => d.agreementCount) ?? [1]),
  );
      ...
      <div className={styles.directionBody}>
        {stats?.topDirections?.map((dr: any, index: number) => (
          <div
            ...
            style={
              {
                '--bar-width': `${Math.round((dr.agreementCount / maxCount) * 100)}%`,
              } as React.CSSProperties
            }
          >
            <div
              className={`${styles.directionRank} ${index === 0 ? styles.rank1 : index === 1 ? styles.rank2 : index === 2 ? styles.rank3 : styles.rankOther}`}
            >
              <span>{index + 1}</span>
            </div>
