...
const UsersFilter = ({
  ...
  const activeCount = [isByDirection, isByRole, accountState].filter(
    Boolean,
  ).length;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.sectionsBody}>
        {/* Role */}
        <div
          className={`${styles.filterSection} ${isByRole ? styles.filterSectionActive : ''}`}
        >
          ...
          <div
            className={`${styles.sectionBody} ${isByRole ? styles.sectionBodyVisible : ''}`}
          >
            <div className={styles.roleChips}>
              {ROLE_OPTIONS.map(({ value, label, Icon }) => {
                ...
                return (
                  ...
                  >
                    <div className={styles.roleChipIcon}>
                      <Icon sx={{ fontSize: 16 }} />
                    </div>
                    <span>{label}</span>
                    {isSelected && isByRole && (
                      <CheckOutlinedIcon
                        sx={{ fontSize: 11 }}
                        className={styles.roleChipCheck}
                      />
                    )}
                  </button>
                );
            ...
            {isByRole && (role === 'ADMIN' || role === 'JURIDICAL') && (
              <div className={styles.roleNote}>
                Les administrateurs et juridiques ne sont pas rattachés à une
                direction.
              </div>
            )}
          </div>
        ...
        {/* Direction */}
        <div
          className={`${styles.filterSection} ${isByDirection ? styles.filterSectionActive : ''} ${directionLockedByRole ? styles.filterSectionLocked : ''}`}
        >
          <div className={styles.sectionHeader}>
            <div className={styles.sectionHeaderLeft}>
              <div className={styles.sectionIcon}>
              ...
              {directionLockedByRole && (
                <span className={styles.lockedPill}>N/A</span>
              )}
            </div>
            <label
              className={`${styles.toggle} ${directionLockedByRole ? styles.toggleDisabled : ''}`}
            >
              <input
                type="checkbox"
                checked={isByDirection}
                disabled={directionLockedByRole}
                onChange={handleByDirectionChange}
              />
              <div className={styles.toggleTrack} />
              <div className={styles.toggleThumb} />
            </label>
          ...
          >
            ...
            <FormControl fullWidth size="small" disabled={!isByDirection}>
              ...
              >
                {getDepartementsFromDirections().map((dp, i) => (
        ...
        {/* État compte */}
        <div
          className={`${styles.filterSection} ${accountState ? styles.filterSectionActive : ''}`}
        >
          <div className={styles.sectionHeader}>
            <div className={styles.sectionHeaderLeft}>
              <div className={styles.sectionIcon}>
                <ToggleOnOutlinedIcon sx={{ fontSize: 15 }} />
              </div>
          ...
          <div
            className={`${styles.sectionBody} ${accountState ? styles.sectionBodyVisible : ''}`}
          >
