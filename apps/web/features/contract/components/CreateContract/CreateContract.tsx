...
const CreateContract = ({ handleClose, type = 'contract' }: Proptype) => {
  ...
  const [
    ...
    setIsAgreementDocumentFileUploading,
  ] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    if (agreementDocumentFile) {
      ...
      const res = await uploadFile({
        ...
        onProgress: (p) => setFileUploadProgress(p),
      });
      url = res.filename;
    ...
    <div className={styles.container}>
      <Stack direction="row">
        <Typography className={styles.title}>
          {type === 'contract' ? 'Nouveau contrat' : 'Nouvelle convension'}
        </Typography>
      </Stack>
            ...
            >
              <Button onClick={() => setVendorModalOpen(true)}>
                ...
                >
                  ...
                  <Typography
                    sx={{ fontSize: '13px', textTransform: 'initial' }}
                  >
                    {vendor
                      ? 'Raison sociale: ' + vendor.company_name
                      : 'aucun fournisseur selectionee'}
                  </Typography>
                </Stack>
              </Button>
            </Stack>
        ...
        {activeStep === 3 && (
          <Stack alignItems="center" gap={1}>
            {isAgreementDocumentFileUploading ? (
              ...
              <>
                <Typography>
                  Creation de {type === 'contract' ? 'contrat' : 'convension'}
                  ...
                </Typography>
                <CircularProgress />
              </>
              ...
              <>
                <Typography>
                  {type === 'contract' ? 'contrat' : 'convension'} cree !
                </Typography>
                <Fab
                  aria-label="save"
                  color="secondary"
      ...
      <Modal open={vendorModalOpen} onClose={() => setVendorModalOpen(false)}>
        <SelectVendor
          handleClose={() => setVendorModalOpen(false)}
          selectVendor={(v: any) => setVendor(v)}
        />
      </Modal>
    </div>
  );
