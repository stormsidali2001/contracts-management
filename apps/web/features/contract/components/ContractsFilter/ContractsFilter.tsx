...
const STATUS_OPTIONS: {
  ...
  },
  {
    value: 'in_execution',
    label: 'En cours',
    bg: 'rgba(23,73,142,0.08)',
    color: '#17498E',
    border: 'rgba(23,73,142,0.28)',
  },
  {
    value: 'executed_with_delay',
...
const ContractsFilter = ({
  ...
  const shouldDisplayFilter = () =>
    permissions?.filters.canFilterByDirection ?? false;
  ...
  const handleDirectionChange = (event: SelectChangeEvent) => {
    ...
    const departement =
      direction.departements[
        Math.floor(Math.random() * direction.departements.length)
      ];
    setSelectedDepartement({
      label: departement?.title,
      value: departement.id as string,
    });
  };
  ...
  const handleChangeDepartement = (event: SelectChangeEvent) => {
    ...
    const directionIndex = departements.findIndex(
  ...
  useEffect(() => {
    if (initialFilters.directionId && initialFilters.departementId) {
      ...
      const selDir = directions.find(
        (d) => d.id === initialFilters.directionId,
      );
      setSelectedDirection({
        label: selDir?.abriviation ?? '',
        value: initialFilters.directionId,
      });
      const selDep = selDir?.departements.find(
        (dp) => dp.id === initialFilters.departementId,
      );
      setSelectedDepartement({
        value: initialFilters.departementId,
        label: selDep?.abriviation ?? '',
      });
    }
    if (
      initialFilters.amount_min !== undefined &&
      initialFilters.amount_max !== undefined
    ) {
      setIsByAmount(true);
  ...
  const activeCount = [isByDirection, isByDate, isByStatus, isByAmount].filter(
    Boolean,
  ).length;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.sectionsBody}>
        ...
        {shouldDisplayFilter() && (
          <div
            className={`${styles.filterSection} ${isByDirection ? styles.filterSectionActive : ''}`}
          >
            <div className={styles.sectionHeader}>
              <div className={styles.sectionHeaderLeft}>
                ...
                <span className={styles.sectionTitle}>
                  Direction et département
                </span>
              </div>
              <label className={styles.toggle}>
                <input
                  type="checkbox"
                  checked={isByDirection}
                  onChange={() => setIsByDirection((v) => !v)}
                />
            ...
            <div
              className={`${styles.sectionBody} ${isByDirection ? styles.sectionBodyVisible : ''}`}
            >
              ...
              <FormControl fullWidth size="small" disabled={!isByDirection}>
                ...
                >
                  {getDepartementsFromDirections().map((dp: any, i: any) => (
                    <MenuItem value={dp?.id} key={i}>
                      {dp?.abriviation}
                    </MenuItem>
                  ))}
                </Select>
        ...
        {/* Date */}
        <div
          className={`${styles.filterSection} ${isByDate ? styles.filterSectionActive : ''}`}
        >
          <div className={styles.sectionHeader}>
            <div className={styles.sectionHeaderLeft}>
              <div className={styles.sectionIcon}>
          ...
          <div
            className={`${styles.sectionBody} ${isByDate ? styles.sectionBodyVisible : ''}`}
          >
        ...
        {/* Status */}
        <div
          className={`${styles.filterSection} ${isByStatus ? styles.filterSectionActive : ''}`}
        >
          <div className={styles.sectionHeader}>
            <div className={styles.sectionHeaderLeft}>
              <div className={styles.sectionIcon}>
          ...
          <div
            className={`${styles.sectionBody} ${isByStatus ? styles.sectionBodyVisible : ''}`}
          >
            <div className={styles.statusChips}>
              {STATUS_OPTIONS.map((opt) => {
                const isActive =
                  isByStatus && selectedStatus.value === opt.value;
                return (
                  <button
                    key={opt.value}
                    ...
                    disabled={!isByStatus}
                    onClick={() =>
                      setSelectedStatus({ value: opt.value, label: opt.value })
                    }
                  >
        ...
        {/* Amount */}
        <div
          className={`${styles.filterSection} ${isByAmount ? styles.filterSectionActive : ''}`}
        >
          <div className={styles.sectionHeader}>
            <div className={styles.sectionHeaderLeft}>
              <div className={styles.sectionIcon}>
          ...
          <div
            className={`${styles.sectionBody} ${isByAmount ? styles.sectionBodyVisible : ''}`}
          >
